...
5. **AI SDK integration** for Vercel AI SDK / OpenAI / Anthropic function calling
...
| Sandbox API | Vercel Sandbox compatible | Vercel Sandbox compatible |
| AI SDK | `bash-tool` package | Built-in `createBashTool()` |
| Browser | ✅ | ✅ (WASM) |
...
### 2.6 — Vercel Sandbox Compatible API

Implement the `Sandbox` class matching Vercel's Sandbox interface:
...
This enables the same migration path as just-bash: start with `@rust-bash/core` for development, swap to `@vercel/sandbox` for production VMs.

**Tests**: Port Sandbox API tests from just-bash.

**Docs**: Add `docs/recipes/sandbox-api.md`.

### 2.7 — Documentation Update (Phase 2)

- Update `README.md`: Add npm installation, TypeScript quick start, `defineCommand` example
- Update `docs/guidebook/08-integration-targets.md`: Full npm package documentation (reconcile with the existing planned `createSandbox()` API — the `Bash` class API is better and replaces it)
- Add `docs/recipes/wasm-usage.md`: WASM-specific guide
- Add `docs/recipes/sandbox-api.md`: Vercel Sandbox compatible API guide
...
**Goal**: First-class Vercel AI SDK tool + generic LLM tool definitions.

### 3.1 — `createBashTool()` for Vercel AI SDK

Create `packages/ai-tool/` (or export from `@rust-bash/core`):

```typescript
import { tool } from 'ai';
import { Bash, type BashOptions } from '@rust-bash/core';

export function createBashTool(options?: BashOptions & {
  /** Maximum output length before truncation */
  maxOutputLength?: number;
  /** Whether to include env in response */
  includeEnv?: boolean;
}) {
  ...
  return {
    tools: {
      bash: tool({
        description: 'Execute bash commands in a sandboxed environment with an in-memory virtual filesystem. Supports 80+ commands including grep, awk, sed, jq, curl, and full bash syntax.',
        parameters: z.object({
      ...
      readFile: tool({
        ...
        execute: async ({ path }) => bash.readFile(path),
      }),
      writeFile: tool({
    ...
    bash,  // expose for advanced use
  };
}
```

This is a drop-in replacement for `bash-tool` (the just-bash AI SDK wrapper):
```typescript
// Before (just-bash):
import { createBashTool } from 'bash-tool';
// After (rust-bash):
import { createBashTool } from '@rust-bash/ai-tool';
```

**Tests**: Integration tests with AI SDK `generateText()`.

**Docs**: Update `docs/recipes/ai-agent-tool.md` with full AI SDK examples.

### 3.2 — Generic LLM Tool Schemas

Export tool schemas for OpenAI, Anthropic, and generic function-calling APIs:

```typescript
import { getToolSchemas } from '@rust-bash/core';

// OpenAI format
const openaiTools = getToolSchemas('openai');
// [{ type: "function", function: { name: "bash", description: "...", parameters: {...} } }]

// Anthropic format  
const anthropicTools = getToolSchemas('anthropic');
// [{ name: "bash", description: "...", input_schema: {...} }]
```

Also export a `handleToolCall(bash, toolName, args)` helper for easy integration into custom agent loops.

**Tests**: Validate schema output against OpenAI/Anthropic specs.

**Docs**: Add examples for OpenAI, Anthropic, and custom agent loops.

### 3.3 — Documentation Update (Phase 3)

- Update `README.md`: AI SDK quick start section
- Add `docs/recipes/ai-agent-tool.md`: Complete AI SDK guide with examples
...
Server-side AI agent using rust-bash native addon:
...
export async function POST(req: Request) {
  ...
  const bashToolkit = createBashTool({
    files: projectFiles,  // rust-bash source code
  });

  const agent = new ToolLoopAgent({
    model: 'claude-haiku-4-5',
    instructions: SYSTEM_INSTRUCTIONS,
    tools: { bash: bashToolkit.tools.bash },
...
The `agent` custom command in the browser terminal sends messages to this endpoint via SSE, displaying tool calls and responses in real-time — exactly like justbash.dev.

**System instructions**: Tailored to rust-bash — explain what it is, how to use it, guide users through exploring the source code.

**Tests**: Test agent endpoint with sample queries.

**Docs**: Document the architecture in `examples/website/README.md`.
...
- **Package naming**: `@rust-bash/core` (main package), `@rust-bash/ai-tool` (AI SDK tool)
...
- `docs/recipes/ai-agent-tool.md` — Full AI SDK examples with actual working code
...
Phase 3 (AI SDK):
  3.1 (createBashTool) ──── 3.2 (tool schemas) ──── 3.3 (docs)
...
- Phase 3 can start as soon as Phase 2.2 is done
...
| AI SDK API churn | Low | Medium | Pin `ai` package version; abstract behind our own tool creator |
| Next.js WASM loading complexity | Medium | Low | Use `next.config.ts` webpack config for `.wasm` files; test in dev + prod |
| Vercel Sandbox API divergence | Low | Low | Our Sandbox is a subset; track upstream changes |
...
4. **`createBashTool()`** provides a drop-in replacement for `bash-tool` (just-bash's AI SDK wrapper)
