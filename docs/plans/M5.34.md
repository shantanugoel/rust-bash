...
- **Chosen approach**: Separate `wasm-bindgen` (WASM) + `napi-rs` (native Node.js addon) builds, unified under a single `@shantanugoel/rust-bash` npm package that auto-detects the environment.
...
- **Package structure**: Single `@shantanugoel/rust-bash` package with conditional exports — native addon for Node.js, WASM for browsers/edge. No need for users to choose between packages.
...
**Goal**: Publish `@shantanugoel/rust-bash` — a TypeScript-first npm package with API parity (and beyond) vs just-bash.
...
try {
  require.resolve('@shantanugoel/rust-bash/native');
  backend = 'native';
} catch {
  backend = 'wasm';
}
...
Export universal building blocks from `@shantanugoel/rust-bash` that work with **any** AI agent framework. The core exports a JSON Schema tool definition and a handler factory — no framework dependencies.

```typescript
// @shantanugoel/rust-bash — the universal primitive
import { bashToolDefinition, createBashToolHandler } from '@shantanugoel/rust-bash';
...
import { createBashToolHandler, bashToolDefinition } from '@shantanugoel/rust-bash';
...
import { tool } from 'ai';
import { z } from 'zod';
import { createBashToolHandler } from '@shantanugoel/rust-bash';
...
**Key design decision**: Framework adapters are documented as recipes (~8 lines each), NOT shipped as separate packages with hard dependencies. This keeps `@shantanugoel/rust-bash` dependency-free (no `ai`, no `zod`, no `@langchain/*`) and works with any framework, including future ones we haven't heard of yet.
...
```
examples/website/
├── package.json              # Vite + @shantanugoel/rust-bash + @xterm/xterm
├── tsconfig.json
...
import { Bash } from '@shantanugoel/rust-bash/browser';
...
import OpenAI from 'openai';
import type { Bash } from '@shantanugoel/rust-bash/browser';
import { bashToolDefinition } from '@shantanugoel/rust-bash';
...
- **Package naming**: `@shantanugoel/rust-bash` (single package — includes Bash class, tool definitions, and handler factory)
...
- Phase 4.3 agent loop uses `bashToolDefinition` from `@shantanugoel/rust-bash` (Phase 2.2+) but the CF Worker proxy is independent — can prototype the Worker + client-side agent loop early using hardcoded tool schemas
...
1. **`npm install @shantanugoel/rust-bash`** works and provides a TypeScript API matching just-bash's `Bash` class
