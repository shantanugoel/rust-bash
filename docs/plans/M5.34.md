...
|         ~500-1500 RPD, native function calling + SSE             |
...
```
examples/website/
├── package.json              # Vite + @rust-bash/core + @xterm/xterm
├── tsconfig.json
...
import { Bash } from '@rust-bash/core/browser';
...
async function handleInput(line: string) {
  ...
  if (trimmed.startsWith('agent ') || trimmed === 'agent') {
    ...
    if (!query) {
      ...
      term.writeln('Example: agent "write a fibonacci script"');
      showPrompt();
      return;
    }
...
async function handleAgentQuery(query: string) {
  ...
  for await (const event of runAgentLoop(query, bash)) {
    switch (event.type) {
      case 'text':
        // LLM streaming text — render inline
        term.write(event.content);
        break;
      case 'tool_call':
        // Show agent-executed command: indented + dimmer to distinguish from user commands
        term.writeln('');
        term.writeln(`  \x1b[2m$\x1b[0m \x1b[36m${event.command}\x1b[0m`);
        if (event.result.stdout) {
          // Indent output, truncate at 50 lines
...
function showPrompt() {
  term.write('\x1b[34muser@rust-bash\x1b[0m:\x1b[36m~\x1b[0m$ ');
}
```

**Welcome message** (rendered on load):
```
  ____  _   _ ____ _____     ____    _    ____  _   _
 |  _ \| | | / ___|_   _|   | __ )  / \  / ___|| | | |
 | |_) | | | \___ \ | |_____|  _ \ / _ \ \___ \| |_| |
 |  _ <| |_| |___) || |_____| |_) / ___ \ ___) |  _  |
 |_| \_\\___/|____/ |_|     |____/_/   \_\____/|_| |_|

 A sandboxed bash interpreter, in Rust. Running in your browser via WASM.

 Try:  ls          cat README.md          echo "hello" | wc -c
       agent "explore this project"
```

The prompt is `user@rust-bash:~$` — looks like a real shell. No splash screen, no loading spinner over the terminal. If WASM isn't loaded yet, queue keystrokes and show `⏳ Loading shell...`, then execute once ready (<2s on a decent connection). Kill the matrix rain / CRT effects from the coming-soon page — they're cool for a teaser, distracting for an interactive tool.
...
```
user@rust-bash:~$ agent "what languages are available?"

🤖 Let me check what's available in this environment...

  $ which python python3 node go rustc          ← indented, dimmer $
  (none found)

  $ ls /usr/lib/
  python3/  rustc/

Based on what I found, you have **Python 3** and **Rust** available.
...
A minimal ~30-line Cloudflare Pages Function that proxies requests to Google Gemini 2.5 Flash (free tier). The API key is stored as a Worker secret, never exposed to the client.
...
- **Primary**: Google Gemini 2.5 Flash (free tier, ~500–1500 RPD, best quality)
...
import OpenAI from 'openai';
import type { Bash } from '@rust-bash/core/browser';
import { bashToolDefinition } from '@rust-bash/core';
...
- Phase 4.3 agent loop uses `bashToolDefinition` from `@rust-bash/core` (Phase 2.2+) but the CF Worker proxy is independent — can prototype the Worker + client-side agent loop early using hardcoded tool schemas
...
5. **Showcase website** at `examples/website/` runs rust-bash WASM in the browser with an AI agent, deployed on Cloudflare Pages ($0/month), using Gemini 2.5 Flash free tier with Workers AI/Groq fallback
